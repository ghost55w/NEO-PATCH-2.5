...
ovlcmd({
  ...
  const { arg, repondre, prenium_id, getJid } = cmd;
  if (!prenium_id) return repondre("⚠️ Seuls les membres de la NS peuvent enregistrer un joueur.");
  const lid = arg[0]?.replace(/@/g, "");
  const mention = await getJid(lid, ms_org, ovl);
...
ovlcmd({
  ...
  const { arg, auteur_Message, prenium_id, repondre, getJid } = cmd_options;
  let userId = auteur_Message;
  if (arg.length >= 1) {
    const lid = arg[0]?.replace(/@/g, "");
    userId = await getJid(lid, ms_org, ovl);
...
ovlcmd({
  ...
  const { arg, repondre, auteur_Message, getJid } = cmd_options;
  const lid = arg[0]?.replace(/@/g, "");
   const userId = await getJid(lid, ms_org, ovl);
...
ovlcmd({
  ...
  const { arg, repondre, auteur_Message, getJid } = cmd_options;
  const lid = arg[0]?.replace(/@/g, "") || auteur_Message;
   const userId = await getJid(lid, ms_org, ovl);
  const data = await getLineup(userId);
  if (!data) return repondre("⚠️ Joueur introuvable.");
...
async function stats_lineup(tex, repondre, getJid, ms_org, ovl) {
  const texte = tex.trim().toLowerCase().split(/\s+/);

  if (texte.length === 4 && texte[0].startsWith("@")) {
    const userId = `${texte[0].replace("@", "")}@lid`;
    const lid = texte[0]?.replace(/@/g, "");
    const userId = await getJid(lid, ms_org, ovl);
    const joueurKey = texte[1].toLowerCase();
    if (!/^j\d+$/.test(joueurKey)) return null;
